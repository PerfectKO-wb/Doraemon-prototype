...
| # | 字段名（英文） | 字段名（中文） | 字段 ID | 类型 | 必填 | 可编辑 | 说明 |
|---|---|---|---|---|:---:|:---:|---|
...
| 33 | Depreciation Method（折旧方法） | `depreciation_method` | 下拉选择 | ✅ | 仅 one_time_expense=No 时必填；Straight-line（定額法）/ Declining Balance（定率法） |
| 34 | Depreciation Period（折旧期间） | `depreciation_start` / `depreciation_end` | Month 选择器（Start → End） | ✅ | 仅折旧区展开时必填；折旧开始月和结束月 |
| 35 | Salvage Value（残值） | `salvage_value` | 数字输入 | ✅ | **仅定額法必填**，资产残余价值 |
| 36 | Depreciation Rate（折旧率） | `depreciation_rate` | 数字输入（%） | ✅ | **仅定率法必填**，年折旧率 |
